...
function add()
{
    ...
    $images_mn = uploadPicture('txtPicture_mn', SRV_ROOT . PROTYPE_IMAGES_DIR);
    $mainImage_mn = $images_mn['image'];
    $thumbnailImage_mn = $images_mn['thumbnail']; 
  
    $images_en = uploadPicture('txtPicture_en', SRV_ROOT . PROTYPE_IMAGES_DIR);
    $mainImage_en = $images_en['image'];
    $thumbnailImage_en = $images_en['thumbnail']; 
    
    $sql = "INSERT INTO protype (DateCreated, DateModified,
                Name_mn, Name_en, 
                Image_mn, Image_en, 
                Rank, IsActive)
            VALUES (NOW(), NOW(),
...
function modify()
{
    ...
    {        
        $sql_mn = "SELECT Image_mn FROM protype WHERE id = $ID";
    ...
    {        
        $sql_en = "SELECT Image_en FROM protype WHERE id = $ID";
...
function delete()
{
    ...
    dbQuery("DELETE FROM protype WHERE ID = $ID");    
...
function deletePicture_mn($ID, $IsCheck)
{
    ...
    if (dbNumRows($result))
    {
        ...
        {            
            ...
            $deleted = @unlink(SRV_ROOT. PROTYPE_IMAGES_DIR . $mainImg);
            $deleted = @unlink(SRV_ROOT. PROTYPE_IMAGES_DIR . $thumbImg);
        }
    }
